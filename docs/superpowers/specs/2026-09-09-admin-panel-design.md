# Admin panel: upload folder and assign eone id
...
An operator opens `/__eone/admin` on the same platform origin, types an identifier (`eone-xxxx`), chooses a local folder, and the server writes that tree to `storage/<id>/`. The page lists existing packages and can delete one after confirmation.
...
- Remote/Docker-specific upload flows
...
| Upload form | client component under that route | Directory picker, client-side id check, `FormData` POST |
| Package APIs | Node route handlers under `/__eone/admin/packages` | List, create, delete |
| Package writer | `lib/eone/` (new helpers, existing `isValidEoneId` / `getStorageRoot`) | Disk list/create/delete with path confinement |
...
- `file`: repeated file parts
- `path`: repeated relative paths, same order and count as `file`

Each `path` is the browser `webkitRelativePath` (or equivalent), including the selected folder name: `mysite/index.html`. The server:

1. Normalizes `\` to `/`
2. Drops the first segment (`mysite`)
3. Treats the rest as the path inside the package (`index.html`)

Reject (400, 相对路径不合法) if after that the path is empty, is `.`, contains a `..` segment, is absolute, or resolves outside `<storageRoot>/<id>/`. Do not skip the first-segment strip on the client; a path that is already `index.html` would otherwise become empty.

Empty directories are not sent by the browser and are not created. That matches how static hosts usually work.

Request body limit: **100 MiB** for the entire HTTP body. Over that → HTTP 413, message 包太大.

## Create semantics

1. Validate `id`. Fail → 400 标识不合法.
2. If `file`/`path` missing, length 0, or counts differ → 400 未选择文件.
...
The UI confirms with the browser `confirm()` dialog before calling DELETE.

## Page flow

Chinese copy, same visual language as the guide page.

Top: identifier text field, folder file input (`webkitdirectory`), submit. Client blocks obviously bad ids and an empty file list; the server re-checks.
...
3. Operator types `eone-7`, picks a folder, submits.
4. POST writes `storage/eone-7/` atomically.
...
| No files / path mismatch | 400 | Create |
| Path escape / empty after strip | 400 | Message 相对路径不合法; temp dir removed; target not created |
| Id already a directory | 409 | No writes |
| Body larger than 100 MiB | 413 | |
...
`node:test` next to the new `lib/eone` helpers (temp dirs, not the repo `storage/` fixtures).

Must cover:

- List: only valid package directories; ignore files and names that fail the id pattern
- Create: files land at `<root>/<id>/...` with the selected-folder prefix stripped
...
Do not automate the directory `<input>` in a browser. Manual check:

1. Guide page link opens the admin panel
2. Upload a folder that contains `index.html` under a new id → list shows it; extension using that id serves the site
...
1. Operator can create a package from a folder + typed id without using the filesystem by hand
